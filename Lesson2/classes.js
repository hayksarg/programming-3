...
class SevSatana {
    ...
    bazmanal() {
        ...
        if (vand && this.energy >= this.speed) {
            this.energy = 1;
            var newsevsatana = new SevSatana(vand[0], vand[1], 3);
            sevSatanaArr.push(newsevsatana);
        }
    }
    ...
    mahanal() {
        if (this.energy <= -(this.speed / 2)) {
            matrix[this.y][this.x] = 0;
            for (var i in sevSatanaArr) {
                if (sevSatanaArr[i].x == this.x && sevSatanaArr[i].y == this.y) {
                    sevSatanaArr.splice(i, 1);
                }
            }
        }
    }
